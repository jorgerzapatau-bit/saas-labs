          ...
          <div key={mes.mes_num} className="flex-1 flex items-end justify-center gap-0.5" style={{ height: "100%" }}>
            {sucursales.map((suc, i) => {
              ...
              return (
                <div
                  key={suc.id}
                  className="relative group flex-1 max-w-[20px]"
...
// ── Heatmap diario ────────────────────────────────────────────
function HeatmapDiario({ detalle, sucursales, mesLabel }: { detalle: DetalleDia[]; sucursales: PacientesData["sucursales"]; mesLabel: string }) {
      ...
      <table className="w-full text-xs border-separate border-spacing-y-1">
        <thead>
          <tr>
            ...
            {dias.map((d) => (
              <th key={d} className="text-center text-slate-500 font-normal pb-2 w-7">{d}</th>
            ))}
          </tr>
        </thead>
        <tbody>
              ...
              {dias.map((d) => {
                ...
                return (
                  <td key={d} className="text-center py-1">
                    <div
                      className="mx-auto w-6 h-5 rounded flex items-center justify-center text-[10px] font-medium cursor-default"
...
export default function DashboardClient({ data }: Props) {
  ...
  // Estado de pacientes (se carga por cliente para no bloquear el server component)
  ...
  const loadPacientes = useCallback(async (mesNum: number | null) => {
    ...
    if (mesNum) params.set("mes_num", String(mesNum));
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100">
      ...
      <main className="max-w-7xl mx-auto px-6 py-6 space-y-6">
                  ...
                  <div className="pl-4">
                    ...
                    <div className="grid grid-cols-3 gap-3 mb-4">
                      <div>
                        ...
                        <p className="text-emerald-400 font-semibold text-sm">{fmtK(suc.total_ingresos)}</p>
                      </div>
                      <div>
                        <p className="text-slate-500 text-xs mb-0.5">Gastos</p>
                        <p className="text-rose-400 font-semibold text-sm">{fmtK(suc.total_gastos)}</p>
                      </div>
                      <div>
                        <p className="text-slate-500 text-xs mb-0.5">Margen</p>
                        <p className={`font-semibold text-sm ${margen >= 0 ? "text-blue-400" : "text-amber-400"}`}>{pct(margen)}</p>
                      </div>
                    </div>
                    ...
                    <div className="border-t border-slate-800 pt-3 grid grid-cols-3 gap-3">
                      <div>
                        ...
                        <p className="text-white font-semibold text-sm">
                          {pacLoading ? "…" : (pacSuc?.pacientes ?? 0).toLocaleString()}
                        </p>
                      </div>
                      <div>
                      ...
                      <div>
                        <p className="text-slate-500 text-xs mb-0.5">Ticket prom.</p>
                        <p className="text-white font-semibold text-sm" style={{ color }}>
                          {pacLoading ? "…" : ticketProm ? fmtK(ticketProm) : "—"}
                        </p>
                      </div>
                    </div>
          ...
          <div className="flex items-center justify-between mb-3">
            ...
            <div className="flex gap-1 p-0.5 bg-slate-800 rounded-lg text-xs">
              <button
                ...
                className={`px-2.5 py-1 rounded-md transition-colors ${pacTab === "pacientes" ? "bg-slate-700 text-white" : "text-slate-500 hover:text-slate-300"}`}
              >
                Pacientes
              </button>
              <button
                onClick={() => setPacTab("ingresos")}
                className={`px-2.5 py-1 rounded-md transition-colors ${pacTab === "ingresos" ? "bg-slate-700 text-white" : "text-slate-500 hover:text-slate-300"}`}
              >
                Ingresos
              </button>
            </div>
            ...
            <div className="bg-slate-900 border border-slate-800 rounded-xl p-5 flex flex-col gap-4">
              ...
              <div className="border-t border-slate-800 pt-4 space-y-3">
                {pacTotalesSuc.map((suc, i) => {
                  const pctSuc = totalPacVis > 0 ? (suc.pacientes / totalPacVis) * 100 : 0;
                  return (
                    <div key={suc.id}>
                      <div className="flex items-center justify-between mb-1">
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          <span className="text-slate-600 text-xs">{pctSuc.toFixed(0)}%</span>
                        </div>
                      </div>
                      <div className="w-full bg-slate-800 rounded-full h-1">
                        <div className="h-1 rounded-full" style={{ width: `${pctSuc}%`, backgroundColor: COLORES_SUCURSAL[i] }} />
                      </div>
                    </div>
                  );
                })}
                    ...
                    <table className="w-full text-xs">
                      ...
                      <tbody className="divide-y divide-slate-800">
                        {pacData.meses.sort((a, b) => a.mes_num - b.mes_num).map((m) => {
                          const mesTotal = m.sucursales.reduce((s, r) =>
                            s + (pacTab === "pacientes" ? r.total_pacientes : r.total_ingresos), 0);
                          const isActive = m.mes_num === pacMes;
                          return (
                            <tr
                              key={m.mes_num}
                              onClick={() => {
                                const next = isActive ? null : m.mes_num;
                                setPacMes(next);
                                setHeatOpen(next !== null);
                              }}
              ...
              {heatOpen && (
                <div className="px-5 pb-5 border-t border-slate-800">
                  {pacLoading ? (
                    ...
                    <div className="pt-4">
                      <HeatmapDiario
                        detalle={pacData?.detalleDiario || []}
                        sucursales={pacData?.sucursales || []}
                        mesLabel={mesLabelPac}
                      />
                    </div>
                  )}
                </div>
        ...
        {/* ── Formas de pago + Top proveedores ── */}
                ...
                return (
                  <div key={prov.nombre} className="flex items-center gap-3">
                    ...
                    <div className="flex-1">
                      ...
                      <div className="w-full bg-slate-800 rounded-full h-1">
                        <div className="h-1 rounded-full bg-rose-500/60" style={{ width: `${pctProv}%` }} />
                      </div>
                    </div>
                  </div>
                );
